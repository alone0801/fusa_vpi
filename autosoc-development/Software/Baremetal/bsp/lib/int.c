...
int int_add(unsigned long irq, void (* handler)(void *), void *arg)
{
  ...
  or1k_mtspr(SPR_PICMR, or1k_mfspr(SPR_PICMR) | (0x00000001L << irq));
...
void int_main()
{
  unsigned long picsr = or1k_mfspr(SPR_PICSR);
  unsigned long i = 0;

  or1k_mtspr(SPR_PICSR, 0);
  ...
  while(i < 32) {
    if((picsr & (0x01L << i)) && (int_handlers[i].handler != 0)) {
      ...
      while (or1k_mfspr(SPR_PICSR) & (0x00000001L << i))
#endif
	      or1k_mtspr(SPR_PICSR, or1k_mfspr(SPR_PICSR) & ~(0x00000001L << i));
    }
    i++;
  }
}
